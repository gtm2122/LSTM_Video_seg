...
class StackDecoder2(nn.Module):
    def __init__(self, x_big_channels, x_channels, y_channels, kernel_size=3):
        ...
        self.up_samp = nn.ConvTranspose2d(in_channels=x_channels,out_channels=x_channels,stride=(2,2),kernel_size=(2,2))
        self.relu = nn.ReLU() ## added this 
        self.bn = nn.BatchNorm2d(x_channels)## added this
        self.decode = nn.Sequential(
            ConvBnRelu2d(x_big_channels + x_channels, y_channels, kernel_size=kernel_size, padding=padding,
                         dilation=1, stride=1, groups=1),
            ConvBnRelu2d(y_channels, y_channels, kernel_size=kernel_size, padding=padding, dilation=1, stride=1,
...
class StackDecoder3(nn.Module):
    def __init__(self, x_big_channels, x_channels, y_channels, kernel_size=3):
        super(StackDecoder3, self).__init__()
        padding = (kernel_size - 1) // 2
...
class lstm_enc_dec(nn.Module):
    def __init__(self,input_shape = 256,hidden_units=256,bi_dir=True,hidden_layers=1,dropout=0):
        super().__init__()
        ...
        self.enc_lstm = nn.LSTM(input_size=self.input_shape,hidden_size=self.hidden_units,num_layers=self.hidden_layers,dropout=self.dropout,bidirectional=self.bi_dir)
        self.dec_lstm1 = nn.LSTM(input_size=self.input_shape,hidden_size=self.hidden_units,num_layers=self.hidden_layers,dropout=self.dropout,bidirectional=self.bi_dir)
        self.dec_lstm2 = nn.LSTM(input_size=self.input_shape,hidden_size=self.hidden_units,num_layers=self.hidden_layers,dropout=self.dropout,bidirectional=self.bi_dir)
    ...
    def forward(self,x):
        ...
        x = self.enc_lstm(x)
        reverse_x = self.dec_lstm1(x)
        future_x = self.dec_lstm2(x)

class UNet1024_lstm(nn.Module):
...
class UNet1282(nn.Module):
    def __init__(self, in_shape):
        super(UNet1282, self).__init__()
        C, H, W = in_shape
        # assert(C==3)
        ...
        # 8
        # x_big_channels, x_channels, y_channels
        self.up6 = StackDecoder2(1024, 1024, 512, kernel_size=3)  # 16
        self.up5 = StackDecoder2(512, 512, 256, kernel_size=3)  # 32
        self.up4 = StackDecoder2(256, 256, 128, kernel_size=3)  # 64
        self.up3 = StackDecoder2(128, 128, 64, kernel_size=3)  # 128
        self.classify = nn.Conv2d(64, 1, kernel_size=1, padding=0, stride=1, bias=True)
...
class UNet10242(nn.Module):
    def __init__(self, in_shape):
        super(UNet10242, self).__init__()
        C, H, W = in_shape
        # assert(C==3)
        ...
        # 8
        # x_big_channels, x_channels, y_channels
        self.up6 = StackDecoder2(768, 768, 512, kernel_size=3)  # 16
        self.up5 = StackDecoder2(512, 512, 256, kernel_size=3)  # 32
        self.up4 = StackDecoder2(256, 256, 128, kernel_size=3)  # 64
        self.up3 = StackDecoder2(128, 128, 64, kernel_size=3)  # 128
        self.up2 = StackDecoder2(64, 64, 24, kernel_size=3)  # 256
        self.up1 = StackDecoder2(24, 24, 24, kernel_size=3)  # 512
        self.classify = nn.Conv2d(24, 1, kernel_size=1, padding=0, stride=1, bias=True)
    ...
    def forward(self, x):
        ...
        out = torch.squeeze(out, dim=1)
        return out
class UNet10243(nn.Module):
    def __init__(self, in_shape):
        super(UNet10243, self).__init__()
        C, H, W = in_shape
        # assert(C==3)
        ...
        # 8
        # x_big_channels, x_channels, y_channels
        self.up6 = StackDecoder3(768, 768, 512, kernel_size=3)  # 16
        self.up5 = StackDecoder3(512, 512, 256, kernel_size=3)  # 32
        self.up4 = StackDecoder3(256, 256, 128, kernel_size=3)  # 64
        self.up3 = StackDecoder3(128, 128, 64, kernel_size=3)  # 128
        self.up2 = StackDecoder3(64, 64, 24, kernel_size=3)  # 256
        self.up1 = StackDecoder3(24, 24, 24, kernel_size=3)  # 512
        self.classify = nn.Conv2d(24, 1, kernel_size=1, padding=0, stride=1, bias=True)
    ...
    def forward(self, x):
        ...
        out = torch.squeeze(out, dim=1)
        return out
class UNet1283(nn.Module):
    def __init__(self, in_shape):
        super(UNet1283, self).__init__()
        C, H, W = in_shape
        # assert(C==3)
        ...
        # 8
        # x_big_channels, x_channels, y_channels
        self.up6 = StackDecoder3(1024, 1024, 512, kernel_size=3)  # 16
        self.up5 = StackDecoder3(512, 512, 256, kernel_size=3)  # 32
        self.up4 = StackDecoder3(256, 256, 128, kernel_size=3)  # 64
        self.up3 = StackDecoder3(128, 128, 64, kernel_size=3)  # 128
        self.classify = nn.Conv2d(64, 1, kernel_size=1, padding=0, stride=1, bias=True)
